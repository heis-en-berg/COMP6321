...
def load_and_preprocess_data(filepath, filename):
    # read data (dataset at "https://archive.ics.uci.edu/ml/datasets/Yeast")
    X = np.loadtxt(filepath + filename, usecols=np.arange(1,9), dtype=np.float64)
    y = np.loadtxt(filepath + filename, usecols=[9], dtype=np.str)
