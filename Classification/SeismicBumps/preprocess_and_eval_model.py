...
def load_and_preprocess_data(filepath, filename):
    ...
    class_column = 18
    X_categorical_features = np.loadtxt(filepath + filename, delimiter=',', skiprows=154, dtype=np.str, usecols=categorcal_features)
    X_non_categorical_features = np.loadtxt(filepath + filename, delimiter=',', skiprows=154, dtype=np.int64, usecols=non_categorical_features)
    y = np.loadtxt(filepath + filename, delimiter=',', skiprows=154, dtype=np.str, usecols=class_column)
