...
def load_and_preprocess_data(filepath, filename):
    ...
    X = np.loadtxt(filepath + filename, delimiter=',', usecols = feature_columns_index)
    y = np.loadtxt(filepath + filename, delimiter=',', usecols = label_column_index, dtype=np.str)
