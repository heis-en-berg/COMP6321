...
def load_and_preprocess_data(filename):
    ...
    class_column = 16
    X_categorical_features = np.loadtxt(filename, delimiter=',', skiprows=21, dtype=np.str, usecols=categorcal_features)
    X_non_categorical_features = np.loadtxt(filename, delimiter=',', skiprows=21, dtype=np.float64, usecols=non_categorical_features)
    y = np.loadtxt(filename, delimiter=',', skiprows=21, dtype=np.str, usecols=class_column)
