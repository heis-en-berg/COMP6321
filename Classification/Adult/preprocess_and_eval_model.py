...
def load_and_preprocess_data(filename):
    ...
    categorcal_features = [1,3,5,6,7,8,9]
    non_categorical_features = [0,2,4,10,11,12]
    class_column = 14
    
    X_categorical_features = np.loadtxt(filename, delimiter=',',dtype='str', usecols=categorcal_features)
    X_non_categorical_features = np.loadtxt(filename, delimiter=',', dtype=np.int32, usecols=non_categorical_features)
    y = np.loadtxt(filename, delimiter=',', dtype=np.str, usecols=class_column)
