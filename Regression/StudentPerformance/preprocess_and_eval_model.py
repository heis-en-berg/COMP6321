...
def load_and_preprocess_data(filepath, filename):
    ...
    X_non_categorical_features = np.loadtxt(filepath + filename, delimiter=';', 
                                            dtype=np.int32, 
    ...
    g1 = np.genfromtxt(filepath + filename, delimiter=';', 
                                    dtype='str', 
                                    skip_header=True,
                                    usecols=[30],
                                    converters = {30: decode_str}).reshape(-1,1)
    
    g2 = np.genfromtxt(filepath + filename, delimiter=';', 
                                    dtype='str', 
                                    skip_header=True,
                                    usecols=[31],
                                    converters = {31: decode_str}).reshape(-1,1)
    y = np.loadtxt(filepath + filename, delimiter=';', dtype=np.int64, usecols=class_column, skiprows=1)
