...
def load_and_preprocess_data(filepath, filename):
    ...
    X = np.loadtxt(filepath +  filename, delimiter=',', usecols=range(2, len(cols)), skiprows=1, dtype=np.uint8)
    y = np.loadtxt(filepath + filename, delimiter=',', usecols=[1], skiprows=1)
